...
可视化查看任务状态与日志。调试面板是开发工具，不进入公开组件入口、全局类型、组件总览首批清单与发布宣传；需要时直接从源码路径导入，生产环境保持关闭。
...
3. 生产环境关闭调试面板与详细日志。
4. `LkPreloadDebugger` 不从 `components/index.ts` 导出，避免被误纳入公开组件 API。
5. 调试面板使用 fixed 定位，仅建议开发态启用；App 与小程序端需避免覆盖业务底部栏。
