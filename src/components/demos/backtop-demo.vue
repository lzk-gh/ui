<template>
  <view class="component-demo">
    <demo-block title="在容器内演示（受控模式）">
      <view class="desc">在内部 scroll-view 中演示 Backtop 的出现与回顶（通过 scrollTop 受控）。</view>
      ...
      <lk-backtop :usePageScroll="false" :scrollTop="innerTop" :bottom="'120rpx'" @to-top="innerTop = 0" />
    </demo-block>
  ...
  <lk-backtop :bottom="'320rpx'" :visibilityHeight="600" :duration="500" />
    ...
    <view class="tips">
      ...
      <text>
        说明：本详情页使用 scroll-view 容器滚动，Backtop 监听的是页面滚动（onPageScroll）。如需体验实际效果，
        请在页面级滚动环境（非 scroll-view）中使用，或在业务页中直接引入。
      </text>
    </view>
  </view>
</template>
