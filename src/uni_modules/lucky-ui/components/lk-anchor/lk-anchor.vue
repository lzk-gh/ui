...
onMounted(() => {
  ...
  // #ifdef H5
  try {
    const headings = Array.from(document.querySelectorAll('[data-lk-anchor-target]')) as HTMLElement[];
...
onBeforeUnmount(() => { try { io && io.disconnect && io.disconnect(); } catch (e) {} });
</script>

<template>
  <view class="lk-anchor" :class="{ 'lk-anchor--affix': props.affix }" :style="{ top: props.affix ? props.offsetTop + 'rpx' : undefined }">
    <slot />
  </view>
</template>

<style scoped lang="scss">
.lk-anchor { position: relative; }
.lk-anchor--affix { position: sticky; }
</style>
