...
const props = defineProps({
  ...
  modelValue: { type: [Date, Array, String] as unknown as () => Date | string | [Date | string, Date | string] | Array<Date | string> | null, default: null },
  type: { type: String as () => 'single' | 'range' | 'multiple', default: 'single' },
...
function makeDate(y:number,m:number,d:number){ return new Date(y, m-1, d, 0, 0, 0, 0); }
...
function isInRangeMode(){ return normalizedType.value === 'range'; }
function isInMultipleMode(){ return normalizedType.value === 'multiple'; }
...
const initSelected = isInRangeMode()
  ? (Array.isArray(props.modelValue) ? parseDate(props.modelValue[0] as DateLike) : parseDate(props.modelValue as DateLike))
  : (isInMultipleMode()
      ? (Array.isArray(props.modelValue) && (props.modelValue as any[]).length>0 ? parseDate((props.modelValue as any[])[0] as DateLike) : null)
      : parseDate(props.modelValue as DateLike));
const initBase = initSelected || today;
const cur = ref<{y:number; m:number}>({ y: initBase.getFullYear(), m: initBase.getMonth()+1 });

// 同步外部 v-model 到内部
function syncFromModel(){
  if(isInRangeMode()){
    if(Array.isArray(props.modelValue)){
      internalRange.value = [parseDate(props.modelValue[0] as DateLike), parseDate(props.modelValue[1] as DateLike)];
    }else{
      internalRange.value = [null, null];
    }
  }else if(isInMultipleMode()){
    if(Array.isArray(props.modelValue)){
      internalMultiple.value = (props.modelValue as any[]).map(v=>parseDate(v as DateLike)).filter(Boolean) as Date[];
    }else{
...
const days = computed(() => {
  ...
  const singleSelected = !isInRangeMode() && !isInMultipleMode() && internalSingle.value && formatDate(internalSingle.value) === ds;
  ...
  const multiSelected = isInMultipleMode() && internalMultiple.value.some(x => formatDate(x) === ds);
...
function select(day:any){
  ...
  if(isInRangeMode()){
    ...
    if(!start || (start && end)){
      // 重新选择起点
      start = day.date; end = null;
    }else{
      // 选择终点（自动调整顺序）
      if(day.date < start){ end = start; start = day.date; }
      else { end = day.date; }
    }
    // clamp 并校验禁用日期
...
function applyShortcut(type: string){
  ...
  const setSingle = (d: Date | null) => { internalSingle.value = d; };
  const setRange = (s: Date | null, e: Date | null) => { internalRange.value = [s,e]; };

  switch(type){
    case 'today':
      if(isInRangeMode()) setRange(todayD, todayD); else setSingle(todayD);
      break;
    case 'yesterday': {
      const yest = new Date(todayD); yest.setDate(yest.getDate()-1);
      if(isInRangeMode()) setRange(yest, yest); else setSingle(yest);
      break;
    }
    case 'last7': {
      const end = todayD;
    ...
    case 'thisMonth': {
      ...
      const e = makeDate(todayD.getFullYear(), todayD.getMonth()+1, new Date(todayD.getFullYear(), todayD.getMonth()+1, 0).getDate());
      if(isInRangeMode()) setRange(s, e); else setSingle(todayD);
      cur.value = { y: s.getFullYear(), m: s.getMonth()+1 };
      break;
    }
    case 'prevMonth': {
      const pm = new Date(todayD); pm.setMonth(pm.getMonth()-1);
      const s = makeDate(pm.getFullYear(), pm.getMonth()+1, 1);
      const e = makeDate(pm.getFullYear(), pm.getMonth()+1, new Date(pm.getFullYear(), pm.getMonth()+1, 0).getDate());
      if(isInRangeMode()) setRange(s, e); else setSingle(s);
      cur.value = { y: s.getFullYear(), m: s.getMonth()+1 };
      break;
    }
    case 'clear':
      if(isInRangeMode()) internalRange.value = [null, null]; else internalSingle.value = null;
      emit('clear');
      break;
  }
...
const accentStyle = computed(() => {
  ...
  return { '--lk-calendar-accent': colorVar, '--lk-calendar-accent-bg': bgVar } as any;
});
</script>
...
<template>
  <view class="lk-calendar" :style="accentStyle">
    ...
    <view v-if="showShortcuts" class="lk-calendar__shortcuts">
      ...
      <view v-if="type==='range'" class="lk-calendar__chip" @click="applyShortcut('last7')">近7天</view>
...
.lk-calendar {
  ...
  --text-secondary: var(--lk-color-text-secondary, rgba(0,0,0,.52));
  ...
  &__ym { font-weight: 700; }

  &__shortcuts {
    display:flex; flex-wrap:wrap; gap: 12rpx;
  }
  &__chip {
    padding: 8rpx 16rpx; border-radius: var(--radius-pill);
    background: var(--accent-bg); color: var(--accent);
    font-size: 22rpx;
    &.is-plain { background: transparent; color: var(--text-secondary); border: 2rpx solid var(--accent-bg); }
  ...
  &__cell {
    ...
    &.is-today { box-shadow: 0 0 0 2rpx var(--accent); }
    ...
    // 区间样式
    &.is-in-range { background: rgba(0,0,0,.04); }
    &.is-range-start,
    &.is-range-end { background: var(--accent); color: var(--text-inverse); font-weight:600; }

    &:active:not(.is-empty):not(.is-disabled) { filter: brightness(.92); }
  }
}
</style>
