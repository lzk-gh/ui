...
export function hsvToRgb(hsv: HSV): RGB {
  let { h, s, v, a } = hsv; h = (h%360+360)%360; s = clamp(s,0,1); v = clamp(v,0,1);
  const c = v * s;
...
export function formatFromHSV(hsv: HSV, fmt: 'hex'|'rgb'|'hsv', alpha=false): string {
  if (fmt === 'hsv') return `${Math.round(hsv.h)},${Math.round(hsv.s*100)}%,${Math.round(hsv.v*100)}%${alpha?`,`+(hsv.a??1):''}`;
  const rgb = hsvToRgb(hsv);
  if (fmt === 'rgb') return `rgb${alpha?'a':''}(${rgb.r}, ${rgb.g}, ${rgb.b}${alpha?`, ${hsv.a??1}`:''})`;
...
export function parseToHSV(input: string, fmt: 'hex'|'rgb'|'hsv'): HSV | null {
  try{
    ...
    if (fmt === 'rgb') {
      ...
      const hsv = rgbToHsv({ r,g,b,a }); hsv.a = a; return hsv;
    }
    if (fmt === 'hsv') {
