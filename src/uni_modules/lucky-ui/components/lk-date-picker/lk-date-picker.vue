...
const props = defineProps({
  ...
  value: { type: [Date, Array] as unknown as () => Date | [Date, Date] | Date[] | null, default: null },
  ...
  disabledDate: { type: Function as unknown as () => ((d: Date) => boolean), default: undefined },
  minDate: { type: [Date, String] as unknown as () => Date | string | null, default: null },
  maxDate: { type: [Date, String] as unknown as () => Date | string | null, default: null },
  // 时间精度（仅 *date-time 模式下生效）
  timePrecision: { type: String as () => 'hour' | 'minute' | 'second', default: 'minute' },
});
...
function syncFromValue(){
  if(props.type === 'range' || props.type === 'range-date-time'){
    ...
    draftMultiple.value = Array.isArray(props.value) ? (props.value as any[]).map(v=>parseDate(v)).filter(Boolean) as Date[] : [];
  }else{
    draftSingle.value = parseDate(props.value as any);
  }
}

watch(() => props.value, () => { if(visible.value) syncFromValue(); });
watch(visible, v => { if(v) syncFromValue(); });

// 年月模式
const ymYear = ref<number>(new Date().getFullYear());
const ymMonth = ref<number>(new Date().getMonth()+1); // 1-12
function ymChangeYear(delta:number){ ymYear.value += delta; }
function ymSelectMonth(m:number){ ymMonth.value = m; }

const months = computed(()=>[
  { m:1, label:'1月' },{ m:2, label:'2月' },{ m:3, label:'3月' },{ m:4, label:'4月' },
  { m:5, label:'5月' },{ m:6, label:'6月' },{ m:7, label:'7月' },{ m:8, label:'8月' },
  { m:9, label:'9月' },{ m:10, label:'10月' },{ m:11, label:'11月' },{ m:12, label:'12月' }
]);
...
function confirm(){
  ...
  if(props.type === 'range'){
    ...
    if(d){
      value = new Date(d.getFullYear(), d.getMonth(), d.getDate(), h1.value, m1.value, props.timePrecision==='second'? s1.value : 0);
    } else value = null;
  }else if(props.type === 'range-date-time'){
    const [s,e] = draftRange.value;
    if(s && e){
      const sdt = new Date(s.getFullYear(), s.getMonth(), s.getDate(), h1.value, m1.value, props.timePrecision==='second'? s1.value : 0);
      const edt = new Date(e.getFullYear(), e.getMonth(), e.getDate(), h2.value, m2.value, props.timePrecision==='second'? s2.value : 0);
      value = [sdt, edt];
    } else value = null;
  }else{
...
<template>
  ...
  <lk-button @click="confirm">确定</lk-button>
      ...
      <template v-if="type!=='year-month'">
        <lk-calendar
          :type="type==='multiple' ? 'multiple' : (type==='range' || type==='range-date-time' ? 'range' : 'single')"
          :color="color"
          :first-day="firstDay"
          ...
          :model-value="
            type==='multiple' ? (draftMultiple as any)
            : (type==='range' || type==='range-date-time' ? (draftRange as any) : (draftSingle as any))
          "
          ...
          <view class="lk-date-picker__time">
            <view class="time-row" v-if="type==='date-time' || type==='range-date-time'">
              ...
              <view class="time-cols">
                <view class="col">
                  <view v-for="n in 24" :key="'h1-'+n" class="cell" :class="{ 'is-active': h1===n-1 }" @click="h1=n-1">{{ (n-1).toString().padStart(2,'0') }}</view>
                </view>
                <view class="col" v-if="timePrecision!=='hour'">
                  <view v-for="n in 60" :key="'m1-'+n" class="cell" :class="{ 'is-active': m1===n-1 }" @click="m1=n-1">{{ (n-1).toString().padStart(2,'0') }}</view>
                </view>
                <view class="col" v-if="timePrecision==='second'">
                  <view v-for="n in 60" :key="'s1-'+n" class="cell" :class="{ 'is-active': s1===n-1 }" @click="s1=n-1">{{ (n-1).toString().padStart(2,'0') }}</view>
                </view>
              </view>
            </view>
            <view class="time-row" v-if="type==='range-date-time'">
              ...
              <view class="time-cols">
                <view class="col">
                  <view v-for="n in 24" :key="'h2-'+n" class="cell" :class="{ 'is-active': h2===n-1 }" @click="h2=n-1">{{ (n-1).toString().padStart(2,'0') }}</view>
                </view>
                <view class="col" v-if="timePrecision!=='hour'">
                  <view v-for="n in 60" :key="'m2-'+n" class="cell" :class="{ 'is-active': m2===n-1 }" @click="m2=n-1">{{ (n-1).toString().padStart(2,'0') }}</view>
                </view>
                <view class="col" v-if="timePrecision==='second'">
                  <view v-for="n in 60" :key="'s2-'+n" class="cell" :class="{ 'is-active': s2===n-1 }" @click="s2=n-1">{{ (n-1).toString().padStart(2,'0') }}</view>
                </view>
              </view>
            </view>
          </view>
        ...
        <view class="lk-date-picker__ym">
          ...
          <view class="lk-date-picker__ym-grid">
            <view
              ...
              :key="mm.m"
              class="ym-cell"
              :class="{ 'is-active': ymMonth===mm.m, 'is-disabled': isMonthDisabled(ymYear, mm.m) }"
...
.lk-date-picker__header {
  ...
  .spacer { flex:1; }
  .lk-date-picker__title { font-weight:700; font-size: 30rpx; }
}
...
.lk-date-picker__ym {
  ...
  .nav {
    ...
    background: var(--lk-color-primary-bg-soft); color: var(--lk-color-primary);
    border-radius: var(--lk-radius-pill);
  }
  .ym-text { font-weight:700; }
  &-grid {
  ...
  .ym-cell {
    ...
    &.is-active { background: var(--lk-color-primary); color: var(--lk-color-text-inverse); font-weight:600; }
...
.lk-date-picker__time {
  display:flex; flex-direction:column; gap: 16rpx;
  .time-row { display:flex; align-items:flex-start; gap: 16rpx; }
  .time-label { width: 160rpx; color: var(--lk-color-text-secondary); font-size: 24rpx; line-height: 1.6; }
  .time-cols { display:flex; gap: 12rpx; flex:1; }
  .col {
    flex:1; max-height: 280rpx; overflow:auto; background: var(--lk-color-primary-bg-soft); border-radius: var(--lk-radius-md);
    .cell { padding: 10rpx 16rpx; text-align:center; color: var(--lk-color-text); }
    .cell.is-active { background: var(--lk-color-primary); color: var(--lk-color-text-inverse); font-weight: 600; }
  }
}
</style>
