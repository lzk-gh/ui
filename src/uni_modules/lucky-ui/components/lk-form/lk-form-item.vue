...
import { formContextKey, type FormContext, type FormRule, type ValidateError, type FormItemContext } from './context';
...
async function doValidate(trigger?:'blur'|'change'){
  ...
  if(!list.length){ status.value='success'; msg.value=''; return; }
  const val = form.model[props.prop];
  status.value='validating'; msg.value='';
  const errs:ValidateError[]=[];
  for(const rule of list){
    const m = rule.message || '验证失败';
    if(rule.required){
      const empty = val===undefined||val===null||(typeof val==='string'&&val.trim()==='')||(Array.isArray(val)&&!val.length);
      if(empty){ errs.push({field:props.prop,message:m,rule}); continue; }
    }
    if(rule.min!=null && typeof val==='string' && val.length<rule.min){ errs.push({field:props.prop,message:m,rule}); continue; }
    if(rule.max!=null && typeof val==='string' && val.length>rule.max){ errs.push({field:props.prop,message:m,rule}); continue; }
    if(rule.pattern && typeof val==='string' && !rule.pattern.test(val)){ errs.push({field:props.prop,message:m,rule}); continue; }
    if(rule.validator){
      try{
      ...
      }catch(e:any){ errs.push({field:props.prop,message:e?.message||m,rule}); }
    }
  }
  if(errs.length){ status.value='error'; msg.value=errs[0].message; return Promise.reject(errs); }
  status.value='success'; msg.value='';
}

const itemCtx:FormItemContext = {
  prop: props.prop || undefined,
  setValidateStatus(s,m){ status.value=s; if(m!==undefined) msg.value=m; },
  validate: doValidate,
  reset(){
    if(props.prop && form){
    ...
    status.value='idle'; msg.value='';
  }
};

onMounted(()=>{ requiredMark.value=computeReq(); form?.addField(itemCtx); });
onBeforeUnmount(()=> form?.removeField(itemCtx));
watch(()=>props.required, ()=> requiredMark.value=computeReq());
...
<template>
  <view class="lk-form-item" :class="[`is-${status}`]">
    ...
    <view class="lk-form-item__content">
      <slot />
      <view v-if="(showMessage ?? form?.showMessage)!==false && status==='error' && msg" class="lk-form-item__error">
        {{ msg }}
      </view>
    </view>
  </view>
...
.lk-form-item {
  ...
  &.is-error &__label { color: var(--_error-color); }
}
</style>