...
const emit = defineEmits(['click','update:show','update:modelValue','after-enter','after-leave']);
...
watch(externalShow, v=> {
  if(v){
    if(!display.value){
      display.value = true;
      // 下一帧触发进入动画
      requestAnimationFrame(()=>{ anim.value='enter'; emit('after-enter'); });
    } else {
      anim.value='enter';
      emit('after-enter');
    }
    ...
    if(display.value){
      anim.value='leave';
      setTimeout(()=>{ display.value = false; emit('after-leave'); }, props.duration);
    }
    unlock();
  }
},{ immediate:true });
onMounted(()=> { if(externalShow.value) lock(); });
...
.lk-overlay {
  ...
  background: var(--lk-overlay-bg, rgba(0,0,0,.55));
  display:flex;
  align-items:center;
  justify-content:center;
  // 初始无动画，按状态类处理
  &.is-enter { animation: lk-overlay-in var(--lk-overlay-duration, .24s) ease; }
  &.is-leave { animation: lk-overlay-out var(--lk-overlay-duration, .24s) ease forwards; }
}
@keyframes lk-overlay-in { from{ opacity:0; } to{ opacity:1; } }
@keyframes lk-overlay-out { from{ opacity:1; } to{ opacity:0; } }
</style>