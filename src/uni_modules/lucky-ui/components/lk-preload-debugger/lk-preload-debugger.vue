  ...
  <view v-if="props.visible" class="lk-preload-debugger" :class="positionClass">
    ...
    <view v-if="isExpanded" class="lk-preload-debugger__body">
      ...
      <view class="lk-preload-debugger__actions">
        <view 
          class="action-btn" 
          :class="{ 'action-btn--active': isPaused }"
          @click="togglePause"
        >
          {{ isPaused ? '恢复' : '暂停' }}
        </view>
        <view class="action-btn action-btn--danger" @click="handleClear">
          清空
        </view>
      </view>
      ...
      <!-- 日志列表 -->
      <scroll-view 
        class="lk-preload-debugger__logs" 
        scroll-y
...
import { getPreloadManager, type PreloadStats, type PreloadEventHandler } from '../../core/src/preload';
