...
async function toggle() {
  ...
  if(props.beforeChange){
    ...
    try{
      ...
      if(!ok){ changing=false; return; }
    }catch{ changing=false; return; }
  }
...
<template>
  <view
      class="lk-switch"
      :class="[`lk-switch--${size}`, { 'is-checked': checked, 'is-disabled': disabled, 'is-loading': loading }]"
      @click="onClick"
  >
...
.lk-switch {
  ...
  &__spinner {
    width:60%;
    height:60%;
    border:4rpx solid rgba(0,0,0,.15);
