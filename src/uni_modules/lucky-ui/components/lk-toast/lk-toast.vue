...
<template>
  ...
  >
    <view class="lk-toast__inner">
      ...
      <text class="lk-toast__text"><slot>{{ message }}</slot></text>
    </view>
  </view>
</template>
...
.lk-toast {
  ...
  &__inner {
    ...
    align-items:center;
    gap:16rpx;
    animation: toast-in .26s;
  }
  &.is-leave .lk-toast__inner {
    animation: toast-out .26s forwards;
  }
  &__text { font-size:26rpx; text-align:center; line-height:1.4; }
  &__icon { color:#fff; }
}
@keyframes toast-in { from{ opacity:0; transform:scale(.85);} to{ opacity:1; transform:scale(1);} }
@keyframes toast-out { from{ opacity:1; transform:scale(1);} to{ opacity:0; transform:scale(.85);} }
</style>