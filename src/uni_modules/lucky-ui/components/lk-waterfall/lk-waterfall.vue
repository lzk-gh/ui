...
const virtItems = computed(() => Array.from({ length: Math.max(0, rowsCount.value) }, (_, i) => ({ _row: i })));
...
watch(() => [props.items, props.column, gapPx.value, paddingXPx.value, columnWidth.value], () => {
	// 任何变化都重新布局
	layoutItems();
});
			...
			<template #default="{ start, end, itemHeight }">
				<view class="lk-waterfall__inner" :style="{ padding: '0 ' + paddingXPx + 'px' }">
					<view
						...
						:style="{ height: (end - start) * itemHeight + 'px', position: 'relative' }"
					>
						<template v-for="node in placed">
							...
							>
								<!-- 插槽：对外暴露 item 和其原始索引 -->
								<slot name="item" :item="items[node.index]" :index="node.index" :style="{ width: '100%', height: '100%' }">
									<!-- 默认卡片（占位示例） -->
